...
class InfoGetter:
    ...
    def gather_article_info(self):
        ...
        self.browser.page_should_contain_element(results_loc, "CurrentPage is not a Results page")
        # The page needs a moment to update the searches
        sleep(1)
        articles = self.browser.find_elements('css:li[data-testid="search-bodega-result"]')
        if not articles:
    ...
    def get_date(self, article:WebElement) -> str:
        ...
        internal text.'''
        date_element = article.find_element('css selector', 'span[data-testid="todays-date"]')
    ...
    def donwload_image(self, image_url:str, image_name:str):
        ...
        if image_url and image_name is not None:
            self.request.download(image_url, target_file=os.path.join("output", image_name))
    ...
    def add_to_article_data(
        ...
        article_is_duplicate = (title in self.article_data['title'] and
            date in self.article_data['date'] and
            description in self.article_data['description'] and
            image_name in self.article_data['image'])
